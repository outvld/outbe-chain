...
fn dispatch_get_scheduled_update() {
    with_update(|storage| {
        ...
        .abi_encode();
        let ret_bytes = dispatch(storage.clone(), &get_data, alloy_primitives::Address::ZERO, U256::ZERO).unwrap();
...
fn dispatch_active_version_and_waiting_list() {
    with_update(|storage| {
        ...
        let active_bytes = dispatch(storage.clone(), &active_data, alloy_primitives::Address::ZERO, U256::ZERO).unwrap();
        assert_eq!(
        ...
        .abi_encode();
        let is_active_bytes =
            dispatch(storage.clone(), &is_active_data, alloy_primitives::Address::ZERO, U256::ZERO).unwrap();
        ...
        let list_bytes = dispatch(storage, &list_data, alloy_primitives::Address::ZERO, U256::ZERO).unwrap();
...
fn dispatch_rejects_non_zero_value() {
    with_update(|storage| {
        ...
        let err = dispatch(storage, &data, alloy_primitives::Address::ZERO, U256::from(1)).unwrap_err();
        assert!(matches!(
            err,
...
fn dispatch_rejects_unknown_selector() {
    with_update(|storage| {
        ...
        let err = dispatch(storage, &data, alloy_primitives::Address::ZERO, U256::ZERO).unwrap_err();
